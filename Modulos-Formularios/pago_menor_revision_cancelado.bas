Attribute VB_Name = "pago_menor_revision_cancelado"
Option Explicit

Sub mespago_menor_mesrevision_cancelada()
...
Dim mn As Integer
Dim xl As Integer
Dim tabla_mod_irph As Range
Dim tabla_mod_euribor As Range
...
Set tabla_mod_irph = Worksheets("datos_int_mod").Range("A2:G29")
Set tabla_mod_euribor = Worksheets("datos_int_mod").Range("H2:N29")
plazos = añost_hipoteca * 12
revision = año_actual - año_hipoteca

If primer_pago_mes < mes_revision Then

primer_pago_mes = primer_pago_mes + 3
mes_revision = mes_revision - 9

mn = primer_pago_mes - mes_revision
xl = primer_pago_mes - mn + 1

 años_plazo_fijo = Worksheets("formulario").Range("B9").Value
 año_hipoteca = año_hipoteca + años_plazo_fijo
 plazos = plazos - (años_plazo_fijo * 12)
 
 If años_plazo_fijo > 0 Then
 capital_pendiente_irph = capital_pendiente_actualizada
 capital_pendiente_euribor = capital_pendiente_actualizada
 Else
 capital_pendiente_irph = capital_inicial
 capital_pendiente_euribor = capital_inicial
 End If
  
  For e = año_hipoteca To año_actual - 1
  
   uf = Worksheets("cuadro_amortizacion").Range("A" & Rows.Count).End(xlUp).Row
    
    buscarv_irph = Application.VLookup(e, tabla_mod_irph, xl, False)
    interes_irph_con_diferencial = buscarv_irph + diferencial
    buscarv_euribor = Application.VLookup(e, tabla_mod_euribor, xl, False)
    interes_euribor_con_diferencial = buscarv_euribor + diferencial_sustitutivo
               
    i_irph = interes_irph_con_diferencial / 1200
    ...
    j_euribor = (1 + i_euribor) ^ (-(plazos))
     ...
     For z = 1 To 12
               ...
               interes_irph_cuota = (capital_pendiente_irph * interes_irph_con_diferencial) / 1200
               ...
               End If
                                   ...
                                   Worksheets("cuadro_amortizacion").Cells(uf + z, 9).Value = buscarv_irph
                                   ...
                                   Worksheets("cuadro_amortizacion").Cells(uf + z, 14).Value = buscarv_euribor
